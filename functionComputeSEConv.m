function [SE_MR, SE_RZF, SE_AMMSE, SE_MR_maxmin, SE_RZF_maxmin, SE_AMMSE_maxmin] = ...
    functionComputeSEConv(Rh2,HBar2,H,Hhat,K,M,tau_p,etaa,nbrOfRealizations,poww,SpecNum_h)
...
for k = 1:K
    Rbar = Rh2(:,:,k);
    for ss = 1:SpecNum_h
        Rbar = Rbar + (HBar2(:,k,ss)*HBar2(:,k,ss)');
    end
    Cp(:,:,k) = poww(k)*( Rbar - tau_p*etaa*Rbar*((tau_p*etaa*Rbar+eye(M))\Rbar));
end
Numerr_MR = zeros(K,1);
Denomm_MR = zeros(K,K);
