...
for jj = 1:K
    ...
    if ell>0
        ...
        %%%%%%%%
        [~,indexxx] = sort(channelGaindB_f(:,ell),'descend');
        numInter = 1;
        hHatj2 = [];
        HHatPrimej2 = [];
        countInter = 0;
        for ii = 1:K
            if indexxx(ii)==jj
                ...
                hHatj2 = [ hHatj2; hDom(:,indexxx(ii)) ];
                HHatPrimej2 = [ HHatPrimej2; GBar2(:,posIndexx,ell,1)*diag(fDom(posIndexx,indexxx(ii),ell))];

            end
            if countInter==numInter
                break
            end
        end


        [anglee1, anglee2, anglee3] = RISPhaseShiftSelection(hHatj,HHatPrimejj,hHatj2,HHatPrimej2);
        %%%%%%%%
...
for k = 1:K
    ...
    for ell = 1:L
        for ss = 1:SpecNum_G
            Rbar = Rbar + GBar2(:,:,ell,ss)*AngleDiagMat1(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat1(:,:,ell)'*GBar2(:,:,ell,ss)';
        end
        Rbar = Rbar + trace(RG_RIS2(:,:,ell)*AngleDiagMat1(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat1(:,:,ell)')*RG_BS2(:,:,ell);
    end

    bHat1(:,k,:) = sqrt(tau_p*etaa)*diag(diag(Rbar))*((tau_p*etaa*diag(diag(Rbar))+eye(M))\ReceivedPilot);
    ...
    for ell = 1:L
        for ss = 1:SpecNum_G
            Rbar = Rbar + GBar2(:,:,ell,ss)*AngleDiagMat2(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat2(:,:,ell)'*GBar2(:,:,ell,ss)';
        end
        Rbar = Rbar + trace(RG_RIS2(:,:,ell)*AngleDiagMat2(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat2(:,:,ell)')*RG_BS2(:,:,ell);
    end

    bHat2(:,k,:) = sqrt(tau_p*etaa)*diag(diag(Rbar))*((tau_p*etaa*diag(diag(Rbar))+eye(M))\ReceivedPilot);
    ...
    for ell = 1:L
        for ss = 1:SpecNum_G
            Rbar = Rbar + GBar2(:,:,ell,ss)*AngleDiagMat3(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat3(:,:,ell)'*GBar2(:,:,ell,ss)';
        end
        Rbar = Rbar + trace(RG_RIS2(:,:,ell)*AngleDiagMat3(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat3(:,:,ell)')*RG_BS2(:,:,ell);
    end

    bHat3(:,k,:) = sqrt(tau_p*etaa)*diag(diag(Rbar))*((tau_p*etaa*diag(diag(Rbar))+eye(M))\ReceivedPilot);
    ...
    for ell = 1:L
        for ss = 1:SpecNum_G
            Rbar = Rbar + GBar2(:,:,ell,ss)*AngleDiagMat4(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat4(:,:,ell)'*GBar2(:,:,ell,ss)';
        end
        Rbar = Rbar + trace(RG_RIS2(:,:,ell)*AngleDiagMat4(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat4(:,:,ell)')*RG_BS2(:,:,ell);
    end

    bHat4(:,k,:) = sqrt(tau_p*etaa)*diag(diag(Rbar))*((tau_p*etaa*diag(diag(Rbar))+eye(M))\ReceivedPilot);
    ...
    for ell = 1:L
        for ss = 1:SpecNum_G
            Rbar = Rbar + GBar2(:,:,ell,ss)*AngleDiagMat5(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat5(:,:,ell)'*GBar2(:,:,ell,ss)';
        end
        Rbar = Rbar + trace(RG_RIS2(:,:,ell)*AngleDiagMat5(:,:,ell)*Rbar_f(:,:,k,ell)*AngleDiagMat5(:,:,ell)')*RG_BS2(:,:,ell);
    end

    bHat5(:,k,:) = sqrt(tau_p*etaa)*diag(diag(Rbar))*((tau_p*etaa*diag(diag(Rbar))+eye(M))\ReceivedPilot);
