...
for k = 1:K
    hHat(:,:,k) = pilCoef*diag(diag(Rbar_h(:,:,k)))*((pilCoef^2*diag(diag(Rbar_h(:,:,k)))+eye(M))\ReceivedPilot0(:,:,k));
end
    ...
    for phor = 1:PorHor
        ...
        for ell = 1:L
            for k = 1:K
                ...
                Rbar_n = zeros(M,M,length(posIndexx));
                posind = 0;
                for nn = posIndexx.'
                    posind = posind + 1;
                    for nn2 = posIndexx.'
                        Rbar_n(:,:,posind) = Rbar_n(:,:,posind) + Rbar_f(nn,nn2,k,ell)*RG_RIS2(nn2,nn,ell)*RG_BS2(:,:,ell);
                        for ss = 1:SpecNum_G
                            Rbar_n(:,:,posind) = Rbar_n(:,:,posind) + Rbar_f(nn,nn2,k,ell)*GBar2(:,nn,ell,ss)*GBar2(:,nn2,ell,ss)';
                        end
                    end
                end

                Rbar_lr = sum(Rbar_n,3);

                posind = 0;
                for nn = posIndexx.'
                    posind = posind + 1;

                    Hhat_RIS(:,:,nn,k,ell) = pilCoef*diag(diag(Rbar_n(:,:,posind)))*((eye(M)+pilCoef^2*diag(diag(Rbar_lr)))\ReceivedPilotL(:,:,k,ell));
        ...
        for tt = 1:nbrOfRealizations
            ...
            for ii = 1:K
                if indexxx(ii)==jj
                    ...
                    hHatj2 = [ hHatj2; hHat(:,tt,indexxx(ii)) ];
                    HHatPrimej2 = [ HHatPrimej2; reshape(Hhat_RIS(:,tt,posIndexx,indexxx(ii),ell), [M,length(posIndexx)])];
...
for k = 1:K
    ...
    Hhat_Conv(:,k,:) = sqrt(tau_p*etaa)*Rbar_h(:,:,k)*((tau_p*etaa*Rbar_h(:,:,k)+eye(M))\ReceivedPilot);
