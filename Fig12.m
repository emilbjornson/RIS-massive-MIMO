% This Matlab script generates Figure 12 in the paper:
%
...
Conv =  vec(SE_Conv_RZF_maxmin);
RIS1 = vec(SE_RIS_RZF_maxmin1(:,:,1));
RIS2b = vec(SE_RIS_RZF_maxmin2b(:,:,1));


%% Figure 12

nbrOfPoints = length(Conv);

figure;
hold on; box on; grid on;
set(gca,'fontsize',16);

ppp1 = plot(sort(Conv),linspace(0,1,nbrOfPoints),'k:','LineWidth',2);
ppp2 = plot(sort(RIS1),linspace(0,1,nbrOfPoints),'b-','LineWidth',2);
ppp3 = plot(sort(RIS2b),linspace(0,1,nbrOfPoints),'r--','LineWidth',2);
...
legend({'Conv-mMIMO', 'RIS-mMIMO, Short-Term', 'RIS-mMIMO, Long-Term'   },'Interpreter','Latex','Location','SouthEast');
xlim([0 6]);
